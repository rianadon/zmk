...
#error "USB split transport selected but no devicetree node with compatible 'zmk,usb-split' found"
#endif
...
K_THREAD_DEFINE(zmk_split_usb_central_thread, 2048, central_thread, NULL, NULL, NULL, 7, 0, 0);
...
static int uhc_event(const struct device *dev, const struct uhc_event *const evt) {
    ...
    case UHC_EVT_EP_REQUEST:
        ...
        if (evt->xfer && evt->xfer->buf && evt->xfer->err == 0 && USB_EP_DIR_IS_IN(evt->xfer->ep) &&
            (evt->xfer->ep & 0x7F) == (ZMK_SPLIT_USB_BULK_IN_EP & 0x7F)) {
 ...
 * matching the in-tree max3421e UHC driver pattern. xfer->ep encodes the
 * DATA direction; STATUS direction is implied as the opposite. */
static int issue_setup(const struct usb_setup_packet *setup, struct net_buf *data_buf) {
    ...
    if (k_sem_take(&xfer_done, K_MSEC(CONFIG_ZMK_SPLIT_USB_TRANSFER_TIMEOUT_MS))) {
...
static int get_descriptor(uint8_t type, uint8_t index, uint16_t length, struct net_buf *buf) {
    struct usb_setup_packet setup = {
        .RequestType =
            {
                .recipient = USB_REQTYPE_RECIPIENT_DEVICE,
...
static bool envelope_is_valid(const uint8_t *buf, size_t len) {
    if (len < ZMK_SPLIT_USB_MSG_EXTRA_SIZE + sizeof(struct zmk_split_usb_event_payload)) {
        return false;
    }
...
static void event_dispatch_work(struct k_work *w) {
    ...
    while ((buf = net_buf_get(&rx_evt_fifo, K_NO_WAIT)) != NULL) {
        if (envelope_is_valid(buf->data, buf->len)) {
            ...
            zmk_split_transport_central_peripheral_event_handler(
                &zmk_split_usb_central_inst, env->payload.source, env->payload.event);
        } else {
...
static int send_command(uint8_t source, struct zmk_split_transport_central_command cmd) {
    if (!atomic_get(&enumerated)) {
...
static void bring_up(void) {
    ...
    if (desc_buf != NULL) {
        ...
        if (ret == 0 && desc_buf->len >= 18) {
            ...
            LOG_INF("peripheral VID=0x%02x%02x PID=0x%02x%02x bcdDevice=0x%02x%02x", d[9], d[8],
                    d[11], d[10], d[13], d[12]);
        } else {
            LOG_WRN("GET_DESCRIPTOR(DEVICE) failed: %d (len=%u)", ret, desc_buf->len);
        }
    ...
    (void)arm_bulk_in();
    LOG_INF("bring-up done — peripheral at addr=%u, bulk-IN armed", PERIPHERAL_DEV_ADDR);
}
